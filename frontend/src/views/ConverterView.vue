<template>
  <div
    class="purrs purrs z-30 min-h-fit w-full justify-between rounded-lg bg-rose-300 p-8 shadow-lg dark:bg-fuchsia-950"
    ...
    <form @submit.prevent="submitForm" class="space-y-2">
      <div class="grid grid-cols-1 gap-2">
        <div>
          ...
          <label for="file"
            class="text-overflow-ellipsis mt-1 block w-full transform cursor-pointer overflow-hidden whitespace-nowrap rounded-md border border-gray-300 p-2 transition duration-500 ease-in-out hover:-translate-y-1 hover:scale-105 dark:border-gray-600 dark:bg-gray-700">
      ...
      <button type="submit" :disabled="isLoading"
        class="text-md rounded bg-rose-600 px-2 py-1 text-white dark:bg-fuchsia-600 dark:text-gray-200"
      ...
      <div v-if="image">
        <div class="purrs mt-4 flex space-x-2">
          <button @click="downloadImage"
            class="text-md transform rounded bg-fuchsia-600 px-2 py-1 text-white transition duration-500 ease-in-out hover:-translate-y-1 hover:scale-110 dark:bg-fuchsia-600 dark:text-gray-200">Download
            Image</button>
          <button @click="toggleDetails"
            class="mb-2 mt-2 rounded bg-green-600 px-2 py-1 text-white dark:bg-purple-600 dark:text-gray-200">
            {{ showDetails ? 'Hide Details' : 'Show Details' }}
          </button>
          <button @click="resetZoom"
            class="text-md rounded bg-purple-600 px-2 py-1 text-white dark:bg-fuchsia-600 dark:text-gray-200">Reset
            Zoom</button>
        </div>
        ...
        <div v-show="showDetails"
          ...
          <transition name="slide-fade">
            <div v-show="showDetails" class="details-overlay">
              <div class="relative w-full md:w-1/2  rounded-lg bg-rose-300 p-8 shadow-lg dark:bg-fuchsia-950">
